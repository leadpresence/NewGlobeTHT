...
import com.leadpresence.newglobetht.data.local.mapper.toDomain
import com.leadpresence.newglobetht.data.local.mapper.toDto
import com.leadpresence.newglobetht.data.local.mapper.toEntity
...
override    fun getPupils(): Flow<PagingData<Pupil>> {
        return Pager(
            config = PagingConfig(
                pageSize = 20,
                enablePlaceholders = false
            ),
            remoteMediator = PupilRemoteMediator(api, dao),
    ...
    override suspend fun getPupil(pupilId: Int): Pupil {
        return dao.getPupilById(pupilId).map { it.toDomain() }.firstOrNull()
            ?: throw Exception("Pupil not found")


    }

    override suspend fun createPupil(pupil: Pupil) {
        val response = api.createPupil(pupil.toDto())
        dao.insert(response.toEntity())
    }

    override suspend fun updatePupil(pupil: Pupil) {
        val response = api.updatePupil(pupil.pupilId.toLong(), pupil.toDto())
        dao.update(response.toEntity())
    }

    override suspend fun deletePupil(pupilId: Int) {
        api.deletePupil(pupilId)
        dao.deleteById(pupilId)
    }
